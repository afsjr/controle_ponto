(() => {
  ...
  function render(){const active=qs('.view.active')?.id.replace('view-','')||'ponto'; if(active==='dashboard')renderDashboard(); if(active==='trabalhadores')renderWorkers(); if(active==='auditoria')renderAudit(); if(active==='acompanhamento')renderAcompanhamento(); updateClock();}
  ...
  document.addEventListener('click',e=>{const nav=e.target.closest('[data-view]');if(nav)navigate(nav.dataset.view);const action=e.target.closest('[data-action]');if(!action)return;const a=action.dataset.action;if(a==='go-ponto')navigate('ponto');if(a==='go-treinamento')navigate('treinamento');if(a==='go-trabalhadores')navigate('trabalhadores');if(a==='go-auditoria')navigate('auditoria');if(a==='open-worker-modal')openWorkerModal();if(a==='open-correction')openCorrection(action.dataset.id);if(a==='rh-detalhe')rhDetalhe(action.dataset.id);if(a==='close-modal')closeModal();});
  initPunchForm(); { const rhForm=qs('#rh-login-form'); if(rhForm)rhForm.addEventListener('submit',rhLogin); const rhMes=qs('#rh-mes'); if(rhMes){rhMes.value=fmtMesAtual(); rhMes.addEventListener('change',renderAcompanhamento);} const rhBtn=qs('#rh-atualizar'); if(rhBtn)rhBtn.addEventListener('click',renderAcompanhamento); } qs('#worker-search').addEventListener('input',renderWorkers); qs('#audit-filter').addEventListener('change',renderAudit); qs('#modal-backdrop').addEventListener('click',e=>{if(e.target.id==='modal-backdrop')closeModal();}); render(); qs('#punch-code') && qs('#punch-code').focus(); setInterval(updateClock,1000); setInterval(render,60000);
})();
